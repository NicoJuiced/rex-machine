...
[![PyPI version](https://img.shields.io/pypi/v/tribl?color=%2334D058&logo=pypi&logoColor=white)](https://pypi.org/project/tribl/)
[![Python](https://img.shields.io/pypi/pyversions/tribl?logo=python&logoColor=white)](https://pypi.org/project/tribl/)
[![CI](https://img.shields.io/github/actions/workflow/status/NicolasJULIEN/tribl/ci.yml?branch=main&logo=github&label=CI)](https://github.com/NicolasJULIEN/tribl/actions)
...
*tribl* lets **Claude navigate your repo like an engineer** — reading files, searching code, understanding structure — then extracts structured technical lessons learned (REX) grounded in real code evidence.
...
tribl analyze /path/to/repo -m claude-opus-4-8

# Use verbose mode to see tool calls
tribl analyze /path/to/repo -v
```

## How It Works

tribl works like **Claude Code analyzing your repo** — each AI agent autonomously navigates the codebase, reading files and searching code to build its analysis.

```
  Your Repo              tribl                           Claude API
 ┌──────────┐     ┌────────────────┐
...
3. **tribl executes the tool** locally and returns the result
...
The first version of tribl pre-scanned the repo with Python, assigned static priority scores to files, read the top-N files within a character budget, and sent the entire blob to Claude. This had fundamental limitations:
...
tribl uses the [official Anthropic Python SDK](https://github.com/anthropics/anthropic-sdk-python) (`anthropic`) — not LangChain, CrewAI, AutoGen, or any agent framework. Why:
...
The synthesis agent must output a valid `RexReport` JSON object. tribl forces this via the `tool_use` pattern:
...
tribl uses `anyio` (not raw `asyncio`) for parallel sub-agent execution:
...
Every tribl report is a `RexReport` Pydantic model:
...
```
Problème       →  What was the challenge?
Approches      →  What was tried? What worked, what didn't?
Apprentissages →  What's the takeaway?
```
...
| Level | Meaning |
|-------|---------|
| `fortement_infere` | Strong direct evidence in the code |
| `probable` | Good evidence with minor inference |
| `hypothese_prudente` | Some evidence, significant inference |
| `signal_insuffisant` | Weak signal — flagged as low-confidence |
...
tribl uses a **two-level config** system:

| Level | File | Contains | Commitable? |
|-------|------|----------|-------------|
| **Global** | `~/.config/tribl/config.json` | API keys, provider | No (secrets) |
| **Project** | `.tribl.json` in repo root | Model, output format, options | Yes |
...
### Project Config (`.tribl.json`)

> **Never put API keys or secrets in this file** — it's meant to be committed to your repo. Secrets belong in env vars or `~/.config/tribl/config.json`.
...
| **Read-only** | tribl never modifies the analyzed repository |
...
```bash
# Clone & install
git clone https://github.com/NicolasJULIEN/tribl.git
cd tribl
pip install -e ".[dev]"
...
**tribl** — *because the best lessons are already in your code.*
